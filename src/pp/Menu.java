...
public class Menu extends JFrame implements ActionListener  {
 ...
 public void RadioListener() {
	...
	public void actionPerformed(ActionEvent e) {
	   ...
	   // db.PrintClient();
	    db.addClientInfo("'dude'", "'im'", "'1'", "'845-234-2323'", 10000, 1, "'nah'", "'cityboys'", "'12312'");
	    db.PrintClient();
	    ArrayList<Client> clientList = new ArrayList<Client>();
	   // clientList.add(db.getClientByID(1));
    	System.out.println(db.getClientByID(3));
	    if(src==b){
	    	
	    	int count = 0;
	    	while(count < clientList.size()) {
	    		if(clientList.get(count).getFname().contains(search.getText())) {
	    			if(!list.contains(clientList.get(count))) {
	    				list.addElement(clientList.get(count));
	    				count++;
	    			}
	    		}
	    	}
