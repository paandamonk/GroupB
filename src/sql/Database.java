...
//		// staffInput.addStaffInfo(5, "'Matt'", "'Smith'", 1, "'New York'", "'Male'",
//		// "'10/14/1997'", 23000.540, 6);
//
...
//		// leaseInput.addLeaseInfo(0, 1, "'Thomas'", "'Benedict'", 1, "'30 Archibald
//		// Lane'", "'Kingston'", "'12528'",
//		// "'Apartment'", 3, 750.0, "'Credit'", 1000.0, 1, "'12/5/18'", "'12/1/20'",
//		// "'Two years'");
//
...
//		// propViewInput.addPropViewInfo(1, "'Richard'", "'Bulganari'",
//		// "'012-345-6789'", 10,
...
//	}
	...
	public static ArrayList<BusinessOwner> getBusinessOwnersByID(int ownID) {
		...
		try {
			...
			c = DriverManager.getConnection("jdbc:sqlite:test.db");
			c.setAutoCommit(false);
			System.out.println("Opened database successfullySTAFF");
			...
			while (rs.next()) {
				...
				if (ownID == ownerId) {
					BusinessOwner businessOwner = new BusinessOwner(FName, LName, street, city, postCode, phone,
							staffId, ownerId, businessName, businessType);
	...
	public Client getClientByID(int id) {
		...
		try {
			...
			float maxRent;
			Client client = null;
			System.out.println("Opened database successfullysrtthhertyhdrth");
			...
			while (rs.next()) {
				...
				client = new Client(idNum, fName, lName, type, phone, staffNum, street, city, postCode,
						maxRent);
				System.out.println(idNum);
				return client;
//				if(id == client.getClientIdNum()) {
//					
...
//				}
	...
	public static void printProp(int Num) {
		...
		try {
			...
			c = DriverManager.getConnection("jdbc:sqlite:test.db");
			c.setAutoCommit(false);
			System.out.println("Opened database succedrdthdrthdrssfully");
			...
			while (rs.next()) {
				...
				/*
				 * if(Num == propNum){ Property prp = new Property(street,city,postcode,type,
				 * propNum,rooms,rent,owner); propertiesList.add(prp); } else if(propNum ==
				 * 000){
				 */
...
//return list;*/
	...
	public void PrintClient() {
	      ...
	      try {
	         ...
	         System.out.println("Opened database successfullyPRINTCLIENT");
	         stmt = c.createStatement();
	         ResultSet rs = stmt.executeQuery( "SELECT * FROM CLIENTS" + ";" );
	         String fName, lName, type, phone, street, city, postCode;
	         int idNum, staffNum;
	...
	public static ArrayList<PropView> getPropView() {
		...
		ArrayList<PropView> propViewList = new ArrayList<PropView>();
		...
		try {
			...
			while (rs.next()) {
				...
				PropView propView = new PropView(clientId, fname, lname, phone, propertyId, street, city, postCode,
						viewDate, comments);
	...
	public void addClientInfo(String fname, String lname, String type, String phone, float maxRent, int staffId,
		...
		try {
			...
			System.out.println("Opened database successfullyAddClient");

			stmt = c.createStatement();
			String sql = "INSERT INTO CLIENTS"  + " (FNAME,LNAME,TYPE,PHONE,MAXRENT,STAFFNUM,STREET,CITY,POSTCODE) "
		...
		try {
			...
			while (rs.next()) {
				...
				if (clientId == clientIdInput) {
					Lease lease = new Lease(leaseId, clientId, fname, lname, propertyId, street, city, postCode, type,
							numRooms, monthlyRent, payMethod, deposit, paidDeposit, rentStart, rentEnd, duration);
					leaseList.add(lease);
				}
				/*
				 * else if(clientIdInput == 0) { Lease lease = new Lease(leaseId, clientId,
				 * fname, lname, propertyId, street, city, postCode, type, numRooms,
				 * monthlyRent, payMethod, deposit, paidDeposit, rentStart, rentEnd, duration);
				 * leaseList.add(lease); }
				 */
			}
		} catch (Exception e) {
	...
	/*
	 * if(Num == propNum){ Property prp = new Property(street,city,postcode,type,
	 * propNum,rooms,rent,owner); list.add(prp); } else if(propNum == 000){ Property
	 * prp = new Property(street, city, postcode, type, propNum, rooms, rent,
	 * owner); list.add(prp); System.out.println(list.get(0)); } } } catch (
	 * Exception e ) { System.err.println( e.getClass().getName() + ": " +
	 * e.getMessage() ); System.exit(0); } System.out.println("Dicks"); //return
	 * list;
	 */
}
